# 03: Clean Quality Assessment with FastQC
=================
This directory contains scripts and configuration files to run FastQC on clean sequencing reads as part of a viral inference pipeline using a SLURM job scheduler.

Following read trimming, we'll check to ensure that

1) all adapters are removed (if they even existed in the 1st place) and
2) that the reads are of sufficient quality for assembly.

The launcher script `run_clean_fastqc.sh` submits a job array to the SLURM scheduler, where each job in the array processes a different sample from the input list through the `03_clean_fastqc.slurm` job script. The configuration file `config.sh` allows you to set parameters such as the input list of samples, working directory, and paths to necessary containers and pass them on to the launcher and the job script.

## Files in this directory
- `run_clean_fastqc.sh`: This is the main launcher script that submits the job array to the SLURM scheduler.
- `03_clean_fastqc.slurm`: This is the SLURM job script that runs FastQC on the clean sequencing reads for each sample.
...
2. Run the `run_clean_fastqc.sh` script to submit the job array to the SLURM scheduler.
