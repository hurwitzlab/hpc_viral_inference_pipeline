...
=================
This directory contains scripts and configuration files to run Prokka on high-quality viral inferences as part of a viral inference pipeline using a SLURM job scheduler.

The launcher script `run_prokka.sh` submits a job array to the SLURM scheduler, where each job in the array processes a different sample from the input list through the `07_prokka.slurm` job script. The configuration file `config.sh` allows you to set parameters such as the input list of samples, working directory, and paths to necessary containers and pass them on to the launcher and the job script.

## Files in this directory
- `run_prokka.sh`: This is the main launcher script that submits the job array to the SLURM scheduler.
- `07_prokka.slurm`: This is the SLURM job script that runs prokka on the raw sequencing reads for each sample.
...
2. Run the `run_prokka.sh` script to submit the job array to the SLURM scheduler.
3. Monitor the job status using SLURM commands (e.g., `squeue`, `sacct`, etc.) to check the progress of the prokka analyses.
