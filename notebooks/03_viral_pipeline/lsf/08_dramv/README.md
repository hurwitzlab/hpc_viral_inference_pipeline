...
- `run_dramv.sh`: This is the main launcher script that submits the job array to the LSF scheduler.
- `07_dramv.lsf`: This is the LSF job script that runs DRAM-v on the raw sequencing reads for each sample.
...
2. Run the `run_dramv.sh` script to submit the job array to the LSF scheduler.
3. Monitor the job status using LSF commands (e.g., `bjobs`, `bpeek`, etc.) to check the progress of the DRAM-v analyses.
